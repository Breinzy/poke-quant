-- Database Schema for Sealed Products Support (MINIMAL - ONLY EXISTING COLUMNS)
...
-- 3. Create table for eBay listings of sealed products (EXACT SAME STRUCTURE as ebay_sold_listings)
CREATE TABLE IF NOT EXISTS ebay_sealed_listings (
    id BIGSERIAL PRIMARY KEY,
    sealed_product_id BIGINT REFERENCES sealed_products(id),
    
    -- EXACT SAME COLUMNS as ebay_sold_listings (from your actual code)
    ...
    -- Phase 6 enhanced fields (from your actual code)
    condition_category VARCHAR,
    search_strategy VARCHAR,
    grade_numeric INTEGER,
    
    -- Optional fields (from your actual code)
    condition VARCHAR,
...
-- 6. Create view for combined listings (ONLY COLUMNS THAT ACTUALLY EXIST)
-- Fixed: Cast both UUID and BIGINT to TEXT to match types in UNION
